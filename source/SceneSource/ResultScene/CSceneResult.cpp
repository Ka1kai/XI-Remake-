...
// コンストラクタ
//----------------------------------------------------
CSceneResult::CSceneResult( CSceneChange* change ) : CSceneBase( change )
{
}
...
// ロード
//----------------------------------------------------
bool CSceneResult::Load( void )
{
	// Result文字
	m_pResultValue = new C2DSpriteRHW;
	m_pResResultValueFilePath = RESULT_RESULTVALUE_PNGPATH;
	CResourceManager::GetInstance()->LoadTexture( m_pResResultValueFilePath );
	...
	m_pResPushEnterFilePath = RESULT_PUSHENTER_PNGPATH;
...
{
	CDirectDrawSystem::GetInstance()->DrawSpriteRHW( CResourceManager::GetInstance()->GetTexture( m_pResResultValueFilePath )->Get(),m_pResultValue->GetVertex() );
...
// 初期化
//----------------------------------------------------
void CSceneResult::Initialize( void )
{
	int score = 0;
	int scoreDigit = 0;
	...
	// Result文字
	m_pResultValue->SetVertexPos( 400.0f,70.0f,250.0f,120.0f );

	// Score
	m_pScoreValue->SetVertexPos( scoreValueXPos, scoreValueYPos, scoreValueWidth, scoreValueHeight );
	score = CUserManager::GetInstance()->GetGameScore()->m_record;
	...
	score = CUserManager::GetInstance()->GetGameScore()->m_chainNum;
	...
	score = CUserManager::GetInstance()->GetGameScore()->m_twoChainNum;
	for( int i = pipDigitNum - 1; i == pipDigitNum - 1 || score != 0; -- i )
	{
		m_twoDigitNum ++;
	...
	score = CUserManager::GetInstance()->GetGameScore()->m_threeChainNum;
	for( int i = pipDigitNum - 1; i == pipDigitNum - 1 || score != 0; -- i )
	{
		m_threeDigitNum ++;
	...
	score = CUserManager::GetInstance()->GetGameScore()->m_fourChainNum;
	for( int i = pipDigitNum - 1; i == pipDigitNum - 1 || score != 0; -- i )
	{
		m_fourDigitNum ++;
	...
	score = CUserManager::GetInstance()->GetGameScore()->m_fiveChainNum;
	for( int i = pipDigitNum - 1; i == pipDigitNum - 1 || score != 0; -- i )
	{
		m_fiveDigitNum ++;
	...
	score = CUserManager::GetInstance()->GetGameScore()->m_sixChainNum;
	for( int i = pipDigitNum - 1;i == pipDigitNum - 1 || score != 0; -- i )
	{
		m_sixDigitNum ++;
	...
	CUserManager::GetInstance()->UpdateScore();
}

//----------------------------------------------------
// 終了
//----------------------------------------------------
void CSceneResult::Finalize( void )
{
